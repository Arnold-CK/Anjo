...
def clean_harvests_df(repeated_harvests_df, customers_harvest_df):
    ...
    repeated_harvests_df["data-structures_repeat-size_small-quantity_small_size"] = pd.to_numeric(
        repeated_harvests_df["data-structures_repeat-size_small-quantity_small_size"], errors='coerce').fillna(0)

    repeated_harvests_df["data-structures_repeat-size_big-quantity_big_size"] = pd.to_numeric(
        repeated_harvests_df["data-structures_repeat-size_big-quantity_big_size"], errors='coerce').fillna(0)

    # Now you can safely sum the two columns to create the 'Quantity' column
    repeated_harvests_df["Quantity"] = (repeated_harvests_df["data-structures_repeat-size_small-quantity_small_size"] +
                                        repeated_harvests_df["data-structures_repeat-size_big-quantity_big_size"])

    # Rename columns for clarity
    repeated_harvests_df.rename(columns={
        "data-structures_repeat-structure_name": "Structure",
        "PARENT_KEY": "InstanceID"
    }, inplace=True)

    customers_harvest_df.rename(columns={
    ...
    final_df = final_df[["Date", "Customer", "Structure", "Quantity", "Entered By"]].dropna()
